...
add_bullets(prs.slides[6], [
    ("全链路真实数据", "高德(景点/路线/天气) + 百炼LLM(8Agent+记忆) + 虚拟房价，无假数据"),
...
add_bullets(prs.slides[7], [
    ...
    ("外部数据源", "高德地图(4接口) + 阿里云百炼LLM(qwen3.7-plus)"),
], size=14)
...
add_bullets(prs.slides[10], [
    ("高德地图(1 key 4接口)", "地理编码/POI搜索/路径规划/天气，真实数据"),
    ("阿里云百炼 LLM", "qwen3.7-plus，8 Agent 推理 + 记忆抽取"),
...
add_bullets(prs.slides[11], [
    ...
    ("行程详情亮点", "Agent流程可视化 + 每日行程时间线 + 报告Markdown渲染"),

...
def main():
    ...
    bullets_slide(prs, "十三、测试与验收", [
        ...
        ("RAG 检索评估", "eval_retrieval.py 跑 52 条测试集（覆盖 24 篇语料，含语义换问法）：Hit@5=98.1%、MRR=0.905（系统化评估，非口头宣称）。"),
    ], 17)
    ...
    bullets_slide(prs, "十六、RAG 检索增强生成（2/2）", [
        ...
        ("检索质量评估", "eval_retrieval.py 跑 52 条测试集，计算 Hit@k/MRR：Hit@5=98.1%、MRR=0.905，系统化可复现，非口头宣称。"),
    ...
    pts = [
        ...
        ("RAG 能力", "pgvector + 混合检索(BM25+向量) + 引用溯源 + 多轮对话 + 知识库(24篇/181 chunk) + 评估(Hit@5=98.1%) 完整闭环。"),

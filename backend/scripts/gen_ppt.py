...
def main():
    ...
    bullets_slide(prs, "九、数据源与数据模型", [
        ...
        ("知识库表(1张)", "document_chunks（30篇语料，513 chunk，Vector 768，含城市速览/景点/美食/政策/季节/交通/住宿/注意事项）。"),
    ], 13)
    ...
    bullets_slide(prs, "十三、测试与验收", [
        ...
        ("RAG 检索评估", "eval_retrieval.py 跑 58 条测试集（覆盖 30 篇语料，含语义换问法）：Hit@5=100%、MRR=0.892（系统化评估，非口头宣称）。"),
    ], 17)
    ...
    bullets_slide(prs, "十六、RAG 检索增强生成（1/2）", [
        ...
        ("知识库规模", "document_chunks 表存 30 篇语料、513 个分块（全国地级市 353 城速览 + 7 类主题语料），全部 768 维真向量。"),
    ], 20)
    ...
    bullets_slide(prs, "十六、RAG 检索增强生成（2/2）", [
        ...
        ("检索质量评估", "eval_retrieval.py 跑 58 条测试集，计算 Hit@k/MRR：Hit@5=100%、MRR=0.892，系统化可复现，非口头宣称。"),
    ...
    pts = [
        ...
        ("RAG 能力", "pgvector + 混合检索(BM25+向量) + 引用溯源 + 多轮对话 + 知识库(30篇/513 chunk，全国地级市353城) + 评估(Hit@5=100%) 完整闭环。"),

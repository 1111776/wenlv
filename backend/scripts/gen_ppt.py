...
def main():
    ...
    bullets_slide(prs, "五、项目亮点", [
        ...
        ("性能大幅超标", "QPS 1109(5.5x)、P95 35ms(快8x)、检索0.3ms(快500x)、干预成功率100%"),
        ...
        ("精致前端视觉", "真实风景壁纸 + 状态看板友好化(进度条+步骤卡片)"),
    ], 7, TOTAL)
    ...
    bullets_slide(prs, "八、核心技术实现", [
        ...
        ("安全", "JWT+RBAC(advisor/supervisor) + 注入检测 + 内容过滤 + 工具白名单"),
    ], 10, TOTAL)

    # 九、数据源与数据模型
    bullets_slide(prs, "九、数据源与数据模型", [
        ("高德地图(1 key 6能力)", "地理编码/POI搜索/路径规划/天气/营业时间/景点图片，全真实数据"),
        ("阿里云百炼 LLM", "qwen3.7-flash，8 Agent 推理 + 记忆抽取（Mock 可切真实，压测不烧钱）"),
    ...
    bullets_slide(prs, "十、API 与前端", [
        ("API(5组20+接口)", "认证(register/login) / 行程(create/list/detail/status/agents/plan-file/report/cancel) / 审核(pending/claim/decision) / 记忆(工单7) / 运维(health/metrics)"),
        ("记忆接口(工单7)", "intervene(强干预) / rollback(回滚) / graph(子图) / search(检索) / interventions(历史)"),
        ("前端 6 页面", "登录注册 / 工作台 / 行程列表 / 行程详情 / 审核台 / 记忆图谱"),
        ("行程详情亮点", "景点图片(可放大) + 三餐餐饮 + 营业时间 + 日期 + 状态看板友好化(进度条+步骤卡片)"),
    ...
    bullets_slide(prs, "十一、部署与启动", [
        ...
        ("演示账号", "advisor_demo(顾问) / supervisor_demo(主管)，密码 wenlv123"),
